...
export const createRateRequest = async (req, res) => {
  try {
    ...
    const inserted = await pool.query(
      ...
      RETURNING *
      `,
      [req.user.customer_id, req.user.id, origin, destination, JSON.stringify(freight)]
    );
    const rateRequest = inserted.rows[0];
    // console.log(rateRequest)

    const company = await pool.query(`SELECT company_name FROM customers WHERE id = $1`, [req.user.customer_id]);
    ...
    await notifyDispatchers({
      ...
      message: [`${origin} → ${destination}`, summarizeFreight(freight)].filter(Boolean).join(" | "),
