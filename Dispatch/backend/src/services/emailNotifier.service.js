...
/**
 * Shared branded shell for transactional customer emails
 */
function buildBrandedShell({ subject, badgeText, badgeColor = "#10b981", bodyHtml }) {
  return `
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    ...
    .header { background: #0f172a; color: #ffffff; padding: 24px; text-align: left; }
    .logo { font-size: 20px; font-weight: 900; color: #38bdf8; letter-spacing: -0.5px; }
    .status-badge { display: inline-block; background: ${badgeColor}; color: #ffffff; padding: 4px 12px; border-radius: 9999px; font-size: 11px; font-weight: 800; text-transform: uppercase; margin-top: 10px; font-family: monospace; }
    .content { padding: 24px; line-height: 1.6; font-size: 14px; }
    .summary-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; background: #f8fafc; padding: 18px; border-radius: 12px; border: 1px solid #e2e8f0; margin: 20px 0; }
    .grid-col strong { display: block; font-size: 10px; text-transform: uppercase; color: #64748b; margin-bottom: 2px; }
    .btn { display: inline-block; background: #0284c7; color: #ffffff !important; text-decoration: none; padding: 12px 24px; border-radius: 10px; font-weight: 700; font-size: 13px; text-align: center; margin-top: 10px; }
...
<body>
  <div class="container">
    <div class="header">
      <div class="logo">NISHAN TRANSPORT INC.</div>
      <div style="font-size: 13px; color: #94a3b8; margin-top: 4px;">Live Shipment Notifications</div>
...
/**
 * Branded customer email for load status milestone transitions
 * (Picked Up / In Transit / Delivered / Delayed-Exception)
 */
export function buildLoadMilestoneEmail({ load = {}, milestoneLabel = "Status Update", trackingUrl = "" }) {
  const loadNumber = load.load_number || load.id || "";
  const subject = `${milestoneLabel} — Load NISHAN-${loadNumber}`;
  const customerName = load.customer_name || "Valued Customer";
  const origin =
    load.origin ||
  ...
  const destination =
    ...
    "Destination";
  const isException = /delay|exception/i.test(milestoneLabel);
  const badgeColor = isException ? "#e11d48" : /delivered/i.test(milestoneLabel) ? "#10b981" : "#0284c7";

  const bodyHtml = `
      <p>Hello <strong>${customerName}</strong>,</p>
      <p>Your shipment <strong>NISHAN-${loadNumber}</strong> has a new status: <strong>${milestoneLabel}</strong>.</p>
      <div class="summary-grid">
        <div class="grid-col">
      ...
      ${trackingUrl ? `
      <p>Follow the live milestone timeline, position, and documents for this load anytime:</p>
      <div style="text-align: center; margin: 24px 0;">
        <a href="${trackingUrl}" class="btn">Track This Shipment ➔</a>
      </div>` : ""}
  `;

  return {
    to: load.customer_email || null,
    subject,
    html: buildBrandedShell({ subject, badgeText: `STATUS: ${milestoneLabel.toUpperCase()} • LOAD NISHAN-${loadNumber}`, badgeColor, bodyHtml }),
    text: `${milestoneLabel} — Load NISHAN-${loadNumber}. ${origin} -> ${destination}.${trackingUrl ? ` Track: ${trackingUrl}` : ""}`,
  };
}

/**
 * Branded customer email delivering approved POD/BOL document links
 */
export function buildPodDeliveryEmail({ load = {}, documents = [], trackingUrl = "" }) {
  const loadNumber = load.load_number || load.id || "";
  const subject = `Proof of Delivery Documents — Load NISHAN-${loadNumber}`;
  const customerName = load.customer_name || "Valued Customer";
    ...
    .map(
      (doc) => `
        <li style="margin-bottom: 8px;">
          <a href="${doc.url}" style="color: #0284c7; font-weight: 700;">${doc.name || "Signed Document"}</a>
        </li>`
    )
    .join("");
  ...
  const bodyHtml = `
      <p>Hello <strong>${customerName}</strong>,</p>
      <p>The signed delivery paperwork for shipment <strong>NISHAN-${loadNumber}</strong> has been reviewed and approved by our dispatch team. You can download it here:</p>
      ...
      ${trackingUrl ? `
      <div style="text-align: center; margin: 24px 0;">
        <a href="${trackingUrl}" class="btn">View Shipment Timeline ➔</a>
      </div>` : ""}
      <p style="font-size: 12px; color: #64748b;">Keep these documents for your records. Reply to this email if anything looks incorrect.</p>
  `;

  return {
    to: load.customer_email || null,
    subject,
    html: buildBrandedShell({ subject, badgeText: `DOCUMENTS READY • LOAD NISHAN-${loadNumber}`, badgeColor: "#10b981", bodyHtml }),
    text: `Proof of Delivery documents for Load NISHAN-${loadNumber}: ${documents.map((d) => d.url).join(" , ")}${trackingUrl ? ` | Track: ${trackingUrl}` : ""}`,
  };
}
