...
export function LifecycleBadge({ rank, label }) {
  ...
  return (
    <span
      ...
      className={`inline-flex min-w-0 max-w-full items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide ${
        arrived
...
function StageMarker({ state, rank }) {
  ...
  return (
    <span
      aria-hidden="true"
      className="flex h-5 w-5 flex-none items-center justify-center rounded-full border-2 border-slate-200 bg-white text-[10px] font-bold text-slate-400"
    >
      {rank}
    </span>
  );
...
function StageCard({ stage, rank, currentRank }) {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <span
          className={`text-[11px] font-semibold uppercase tracking-wider ${
            state === "done" ? "text-emerald-700" : state === "current" ? "text-sky-700" : "text-slate-400"
          }`}
        >
          Stage {rank}
        </span>
      ...
      {stage.label && (
        <p
          className={`mt-2 break-words text-sm font-bold leading-snug ${
      ...
      {stage.description && (
        <p className="mt-0.5 break-words text-xs leading-snug text-slate-500">{stage.description}</p>
      )}
      ...
      <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-0.5">
        ...
        {shownDate && <span className="text-[11px] font-medium text-slate-500">{shownDate}</span>}
      </div>
        ...
        <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-slate-200">
          <div
            className={`h-full rounded-full transition-[width] duration-500 ease-out ${
              arrived ? "bg-emerald-500" : "bg-sky-500"
            }`}
            style={{ width: `${percent}%` }}
          />
        </div>
          ...
          {SUMMARY_STEPS.map((step, index) => {
            ...
            return (
              ...
              >
                <span
                  aria-hidden="true"
                  className={`mx-auto block h-3 w-3 rounded-full border-2 ${
                    reached
                      ? arrived
                ...
                />
                <span
                  className={`mt-1.5 block break-words text-[10px] font-semibold uppercase leading-tight tracking-wide sm:text-[11px] ${
                    isActiveStep
                      ? arrived
                        ? "text-emerald-700"
                        : "text-sky-700"
