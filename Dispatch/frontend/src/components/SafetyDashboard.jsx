    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-8 space-y-6">
        ...
        <div className="bg-base-100 rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-200 bg-base-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center space-x-2">
              <AlertTriangle className="text-rose-600 h-5 w-5" />
              <h3 className="text-base font-semibold text-base-content">Samsara Critical Safety Alerts & Incidents</h3>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-y-2 space-x-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-8 pr-3 py-1 bg-base-100 border border-slate-200 rounded-lg text-xs focus:outline-none"
                />
              </div>
              <select
                value={filterSeverity}
                onChange={(e) => setFilterSeverity(e.target.value)}
                className="bg-base-100 border border-slate-200 rounded-lg text-xs px-2 py-1 focus:outline-none"
              >
          ...
          <div className="divide-y divide-slate-100">
            {filteredIncidents.length === 0 ? <div className="text-center text-slate-400 py-12 text-xs">
            ...
            </div> : filteredIncidents.map((inc) => <div key={inc.id} className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 hover:bg-base-200/50 transition-colors">
              <div className="flex items-start space-x-3.5">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center space-x-2">
                    <span className="font-bold text-base-content text-xs">{inc.driverName}</span>
                    <span className="text-slate-400 font-mono text-2xs">•</span>
                    <span className="text-base-content font-mono text-2xs">Truck {inc.truckNumber}</span>
                  ...
                  <div className="flex items-center space-x-4 text-3xs font-mono text-base-content pt-1">
        ...
        <div className="bg-base-100 rounded-xl border border-slate-200 shadow-sm p-5 space-y-4">
          <h3 className="text-sm font-semibold text-base-content">Samsara Real-time ELD Duty Audit</h3>
            ...
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-slate-100">
                {hosLogs.map((log) => {
                  ...
                  return <tr key={log.driverId} className="hover:bg-base-200/50">
                    <td className="px-4 py-3.5">
                      <div className="font-semibold text-base-content">{log.driverName}</div>
            ...
            {scores.map((score) => {
              ...
              return <div key={score.driverId} className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-semibold text-base-content">{score.driverName}</span>
                ...
                <div className="grid grid-cols-3 gap-2 text-3xs font-mono text-base-content pt-1 border-b border-slate-50 pb-2">
