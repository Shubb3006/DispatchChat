...
export default function KanbanColumn({
  ...
  if (isCollapsed) {
    return (
      <div
        ...
        className={`w-14 shrink-0 bg-slate-100/90 border border-slate-200 rounded-2xl p-3 flex flex-col items-center justify-between cursor-pointer hover:bg-slate-200/80 transition-all ${
          isOver ? "ring-2 ring-sky-400 bg-sky-100" : ""
        }`}
      ...
      >
        <div className="flex flex-col items-center gap-3 pt-1">
          ...
          <span className="px-1.5 py-0.5 rounded-full text-[10px] font-mono font-bold bg-white text-slate-800 border border-slate-200 shadow-2xs">
            {shipments.length}
          </span>
        </div>
  ...
  return (
    <div
      ...
      onDragLeave={handleDragLeave}
      onDrop={handleDrop}
      className={`w-84 shrink-0 flex flex-col rounded-2xl border transition-all duration-200 ${
        isOver
    ...
    >
      ...
      <div className="p-3.5 border-b border-slate-200/80 flex items-center justify-between bg-white rounded-t-2xl shadow-2xs">
      ...
      <div className="p-3 flex-1 overflow-y-auto space-y-3 min-h-[520px] max-h-[calc(100vh-270px)] no-scrollbar">
        {sortedShipments.length === 0 ? (
          <div
            className={`h-40 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center p-4 text-center transition-all ${
              isOver
                ? "border-sky-400 bg-sky-100/60 text-sky-800 shadow-inner"
                : "border-slate-300/70 text-slate-400 bg-white/50"
            }`}
          >
            <div className="w-10 h-10 rounded-2xl bg-white flex items-center justify-center shadow-2xs border border-slate-200 mb-2">
