...
const TONES = {
  ...
  mute: "bg-slate-100 text-base-content",
...
function ColumnFilter({ column, options, selected, onChange, onClose }) {
  ...
  return (
    <div
      ...
      className="absolute top-full left-0 mt-1 w-56 bg-base-100 border border-slate-300 rounded-lg shadow-xl z-50 text-slate-800 normal-case tracking-normal font-normal"
    >
      ...
      <div className="max-h-56 overflow-y-auto py-1">
        <label className="flex items-center gap-2 px-2.5 py-1 hover:bg-base-200 cursor-pointer text-[11px] font-bold border-b border-slate-100">
        ...
        {shown.map((opt) => (
          <label key={opt} className="flex items-center gap-2 px-2.5 py-1 hover:bg-base-200 cursor-pointer text-[11px]">
            <input
              type="checkbox"
      ...
      <div className="flex items-center justify-between p-2 border-t border-slate-200 bg-base-200">
        <button onClick={() => onChange(null)} className="text-[10px] font-bold text-slate-600 hover:text-base-content cursor-pointer">
          Clear
        </button>
...
function EditCell({ edit, value, onChange }) {
  ...
  const base =
    "w-full min-w-[110px] px-1.5 py-1 text-[11px] border border-sky-400 rounded bg-base-100 focus:outline-none focus:ring-1 focus:ring-sky-500";
...
export default function LoadOperationsBoard({ shipments, trips, onOpenDetails, onSaveLoad }) {
  ...
  return (
    <div className="bg-base-100 border border-slate-300 rounded-2xl shadow-sm overflow-hidden">
      {/* ── Filter toolbar ── */}
      <div className="px-4 py-2.5 bg-base-200 border-b border-slate-200 flex flex-wrap items-center gap-3 text-[11px]">
        ...
        <select
          ...
          className="px-2 py-1 bg-base-100 border border-slate-300 rounded-lg font-bold text-slate-800 focus:outline-none focus:ring-1 focus:ring-sky-500 cursor-pointer"
        >
        ...
        <input
          ...
          className="px-2 py-1 bg-base-100 border border-slate-300 rounded-lg text-slate-800 focus:outline-none focus:ring-1 focus:ring-sky-500"
        />
      ...
      <div className="overflow-auto max-h-[70vh]">
        <table className="text-left border-collapse text-[11px] whitespace-nowrap">
            ...
            <tr className="text-white uppercase tracking-wide text-[10px] font-black">
              {COLUMNS.map((col) => {
                ...
                return (
                  <th
                    key={col.key}
                    className={`px-3 py-2.5 relative ${col.head} ${col.pinned ? "sticky left-0 z-30 border-r-2 border-slate-500" : ""
                      }`}
                  >
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        className={`p-0.5 rounded cursor-pointer transition ${isFiltered ? "bg-base-100 text-base-content" : "hover:bg-base-100/25 text-white/70"
                          }`}
                      >
          ...
          <tbody className="text-slate-800">
            {tripGroups.length === 0 ? (
              <tr>
                <td colSpan={COLUMNS.length + 1} className="px-5 py-12 text-center text-base-content font-medium">
              ...
              tripGroups.map((group) => (
                <React.Fragment key={group.key}>
                  ...
                  {group.loads.map((s, idx) => {
                    ...
                    const zebra = idx % 2 === 0 ? "bg-base-100" : "bg-base-200";
                      ...
                      >
                        {COLUMNS.map((col) => {
                          ...
                          return (
                            <td
                              key={col.key}
                              className={`px-3 py-2 ${col.mono ? "font-mono" : ""} ${col.className || ""} ${col.pinned ? `sticky left-0 z-10 ${rowBg} border-r-2 border-slate-300 font-mono font-black text-sky-700` : ""
                                }`}
                            >
                              {editable ? (
                                <EditCell
                        ...
                        <td className={`px-3 py-2 sticky right-0 z-10 ${rowBg} border-l-2 border-slate-300`}>
                          {isEditing ? (
                            ...
                            <button
                              ...
                              className="p-1 rounded hover:bg-sky-100 text-base-content hover:text-sky-700 cursor-pointer"
                            >
      ...
      {/* ── Footer ── */}
      <div className="px-4 py-2 bg-base-200 border-t border-slate-200 text-[10px] text-base-content font-medium flex items-center gap-3 flex-wrap">
