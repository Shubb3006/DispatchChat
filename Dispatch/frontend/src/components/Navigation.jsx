...
export default function Navigation({
  ...
  return (
    <>
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          <div
            ...
            aria-hidden="true"
          />
          <aside className="relative w-72 max-w-[85vw] h-full bg-base-100 border-r border-slate-200 shadow-2xl flex flex-col py-4 px-3 overflow-y-auto">
            <div className="flex items-center justify-between mb-4 px-1">
              <div className="flex items-center gap-2.5">
                ...
                <span className="font-extrabold text-base-content text-sm tracking-wide">NISHAN TMS</span>
              </div>
              <button
                onClick={onCloseMobile}
                className="w-8 h-8 flex items-center justify-center rounded-lg text-base-content hover:bg-slate-100 cursor-pointer"
            ...
            <nav className="flex flex-col gap-1 flex-1">
              {filteredTabs.map((tab) => {
                ...
                return (
                  ...
                  >
                    <Icon className={`w-4.5 h-4.5 shrink-0 ${isActive ? "text-sky-600" : "text-base-content"}`} />
      ...
      <aside className="hidden md:flex w-16 flex-col items-center py-4 bg-base-100 border-r border-slate-200 shrink-0 h-full select-none z-30 shadow-xs">
        ...
        <nav className="flex flex-col gap-1.5 w-full items-center flex-1 min-h-0 py-1">
          {filteredTabs.map((tab) => {
            ...
            return (
              ...
              >
                <Icon className={`w-4.5 h-4.5 transition-colors ${isActive ? "text-sky-600" : "text-base-content group-hover:text-base-content"}`} />
