...
export default function InvoicingDashboard({
  ...
  const [calcWeight, setCalcWeight] = useState(6e3);
  ...
  const handleCreateInvoiceFromDoc = (doc) => {
    const matchedShipment = shipments.find((s) => s.id === doc.shipmentId);
    const invoiceId = "INV" + Math.floor(1e4 + Math.random() * 9e4);
    const subtotal = matchedShipment
      ? matchedShipment.priceInvoice
      : Math.round((doc.extractedData?.weightLbs || 5e3) * 0.4);
                          ...
                          <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2 mt-2">
                            ...
                            <div className="flex items-center gap-1.5">
                              <button
                                onClick={() =>
                                  onVerifyDocument &&
                                  onVerifyDocument(doc.id, "rejected")
                                }
                              ...
                              <button
                                onClick={() =>
                                  onVerifyDocument &&
                                  onVerifyDocument(doc.id, "approved")
                                }
                                  ...
                                  <div className="grid grid-cols-3 gap-1">
                                    {doc.skidPictures.map((pic, idx) => (
                                      ...
                                      >
                                        ...
                                        />
                                        <div className="absolute inset-0 bg-slate-950/80 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                          <span className="text-[7px] text-white font-mono font-bold font-mono">
                                            Preview #{idx + 1}
                                          </span>
                                        </div>
                                      </div>
                ...
                <div className="flex items-center justify-between text-slate-600">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      onUpdateInvoiceStatus(selectedInvoice.id, newStatus);
                ...
                <div className="flex items-center justify-between text-slate-600">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const due = new Date(
                        issue.getTime() + days * 24 * 60 * 60 * 1e3
                      );
