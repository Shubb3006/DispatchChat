...
export default function SettlementPaystubModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-in fade-in duration-150 print:p-0 print:bg-base-100">
      <div
        className="bg-base-100 border border-slate-200 rounded-3xl w-full max-w-3xl max-h-[90vh] shadow-2xl flex flex-col overflow-hidden text-base-content animate-in zoom-in-95 duration-150 print:border-0 print:shadow-none print:max-h-none print:w-full"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Top Actions Bar (Hidden on Print) */}
        <div className="px-4 sm:px-6 py-4 border-b border-slate-200 flex flex-wrap items-center justify-between gap-2 bg-base-200 print:hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-6 bg-base-100 text-base-content">
          {/* Header Banner */}
          <div className="flex flex-wrap justify-between items-start gap-4 border-b-2 border-slate-900 pb-6">
            <div>
              <div className="text-xl font-black tracking-tight text-base-content uppercase">
                Nishan Transport Inc.
              </div>
              <div className="text-xs text-base-content font-medium mt-0.5">
                Cross-Border Logistics & Freight Lines
              </div>
              <div className="text-[11px] text-base-content font-mono mt-1 space-x-2">
            ...
            <div className="text-right space-y-1">
              <span className="px-3 py-1 rounded-full text-xs font-black font-mono uppercase bg-slate-100 text-base-content border border-slate-300 inline-block">
                {settlement.status}
              </span>
              <div className="text-base font-black font-mono text-base-content pt-1">
                {settlement.settlement_number}
              </div>
              <div className="text-[11px] text-base-content font-mono">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-base-200 rounded-2xl border border-slate-200 text-xs">
            <div>
              <div className="text-[10px] font-mono font-bold uppercase text-base-content">Driver Name</div>
              <div className="font-extrabold text-base-content mt-0.5">{settlement.driver_name}</div>
              <div className="text-[10px] text-slate-400 font-mono">{settlement.driver_code}</div>
            </div>

            <div>
              <div className="text-[10px] font-mono font-bold uppercase text-base-content">Assigned Power Unit</div>
              <div className="font-extrabold text-base-content mt-0.5">{settlement.truck_number || "TRK-104"}</div>
              <div className="text-[10px] text-slate-400 font-mono">53ft Dry / Reefer</div>
            </div>

            <div>
              <div className="text-[10px] font-mono font-bold uppercase text-base-content">Compensation Model</div>
              <div className="font-extrabold text-base-content mt-0.5">
            ...
            <div>
              <div className="text-[10px] font-mono font-bold uppercase text-base-content">Total Miles Settled</div>
              <div className="font-extrabold text-base-content font-mono mt-0.5">{Number(settlement.total_miles || 0).toLocaleString()} mi</div>
          ...
          <div className="space-y-2">
            <div className="text-xs font-black text-base-content uppercase tracking-wider font-mono">
                    ...
                    {loads.length === 0 ? (
                      ...
                      loads.map((l, idx) => (
                        <tr key={idx} className="hover:bg-base-200">
                          ...
                          <td className="py-2 px-3 text-right font-mono font-bold text-base-content">
            ...
            <div className="space-y-2">
              ...
              <div className="bg-emerald-50/50 border border-emerald-200 rounded-xl p-3.5 space-y-2 text-xs font-mono">
                <div className="flex justify-between">
                  ...
                  <strong className="text-base-content">${Number(settlement.base_pay || 0).toLocaleString()} CAD</strong>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-600">Extra Stop Pay:</span>
                  <strong className="text-base-content">+${Number(settlement.extra_stop_pay || 0).toLocaleString()} CAD</strong>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-600">Detention Wait Pay:</span>
                  <strong className="text-base-content">+${Number(settlement.detention_pay || 0).toLocaleString()} CAD</strong>
                </div>
                {Number(settlement.layover_pay || 0) > 0 && (
                  <div className="flex justify-between">
                    <span className="text-slate-600">Layover Pay:</span>
                    <strong className="text-base-content">+${Number(settlement.layover_pay).toLocaleString()} CAD</strong>
                  </div>
                )}
