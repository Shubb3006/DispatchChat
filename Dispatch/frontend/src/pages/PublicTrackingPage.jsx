...
function Field({ label, value, hint }) {
  return (
    <div className="min-w-0">
      ...
      <p className="mt-1 break-words text-sm font-medium text-base-content">{value || "—"}</p>
      {hint && <p className="mt-0.5 text-xs text-base-content">{hint}</p>}
    </div>
  );
}
...
function Card({ title, icon: Icon, action, children, className = "" }) {
  return (
    <section className={`rounded-xl border border-slate-200 bg-base-100 shadow-sm ${className}`}>
      {title && (
...
function Lane({ origin, destination }) {
  return (
    <span className="inline-flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1">
      <span className="break-words font-semibold text-base-content">{origin || "—"}</span>
      <ArrowRight className="h-3.5 w-3.5 flex-none text-slate-400" />
      <span className="break-words font-semibold text-base-content">{destination || "—"}</span>
    </span>
  );
}
...
function TimelineIcon({ kind }) {
  const map = {
    ...
    document: { Icon: CalendarDays, tone: "bg-slate-100 text-base-content ring-slate-500/20" },
    system: { Icon: CalendarDays, tone: "bg-slate-100 text-base-content ring-slate-500/20" },
  };
  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 space-y-6 px-4 py-6 sm:px-6">
        {[0, 1, 2].map((i) => (
          <div key={i} className="animate-pulse rounded-xl border border-slate-200 bg-base-100 p-5 shadow-sm">
      ...
      <Shell>
        <main className="mx-auto w-full max-w-7xl flex-1 space-y-6 px-4 py-6 sm:px-6">
          <div className="mx-auto w-full max-w-md">
            <section className="rounded-xl border border-slate-200 bg-base-100 p-5 shadow-sm sm:p-6">
              <h1 className="text-lg font-bold tracking-tight text-base-content">Track a shipment</h1>
              ...
              <form onSubmit={handleLookup} className="mt-5 space-y-3">
                <div>
                  <label
                    ...
                    className="mb-1.5 block text-[11px] font-semibold uppercase tracking-wider text-base-content"
                  >
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-base-content placeholder:text-slate-400 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-100"
                  />
                </div>
                <button
                  type="submit"
              ...
              <p className="mt-4 text-xs text-base-content">
  ...
  if (error?.token === token) {
    ...
    return (
      <Shell>
        <main className="mx-auto w-full max-w-7xl flex-1 space-y-6 px-4 py-6 sm:px-6">
          <div className="mx-auto w-full max-w-md">
            <section className="rounded-xl border border-slate-200 bg-base-100 p-5 text-center shadow-sm sm:p-6">
              <span
              ...
              <h1 className="mt-4 text-base font-bold text-base-content">
    ...
    <Shell>
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-end justify-between gap-4 px-4 py-5 sm:px-6">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="break-words text-xl font-bold tracking-tight text-base-content sm:text-2xl">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-base-100 px-3.5 py-2 text-sm font-semibold text-slate-700 transition-colors hover:bg-base-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 disabled:opacity-60"
            >
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 space-y-6 px-4 py-6 sm:px-6">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-sm font-bold text-base-content">{eta?.label || "Delivery date to be confirmed"}</p>
                {/* No date means no number to show — eta.label and eta.detail
                    already say so in words. */}
                {eta?.at && (
                  <>
                    <p className="mt-1 text-2xl font-bold tabular-nums leading-none text-base-content">
                ...
                {eta?.detail && <p className="mt-2 text-sm text-base-content">{eta.detail}</p>}
              </div>
              <Pill className={etaTone(eta?.state)}>{humanize(eta?.state) || "Unknown"}</Pill>
            </div>

            {eta?.at && (
              <p className="mt-4 flex items-start gap-2 rounded-lg bg-base-200 px-3 py-2.5 text-xs leading-relaxed text-slate-600">
          ...
          {showPosition && (
            <Card title="Last reported position" icon={MapPin}>
              {position.place ? (
                <p className="break-words text-sm font-medium text-base-content">{position.place}</p>
              ) : (
              ...
              <p className="mt-3 text-xs leading-relaxed text-base-content">
        ...
        >
          {events.length === 0 ? (
            <div className="rounded-lg border border-dashed border-slate-300 px-4 py-10 text-center">
              <p className="text-sm font-semibold text-base-content">No milestones recorded yet</p>
              <p className="mt-1 text-sm text-base-content">
              ...
              {events.map((event, i) => (
                <li key={`${event.at || "t"}-${event.label}-${i}`} className="flex gap-3">
                  ...
                  <div className={`min-w-0 flex-1 ${i < events.length - 1 ? "pb-5" : ""}`}>
                    <p className="break-words text-sm font-medium text-base-content">{event.label || "—"}</p>
                    {event.detail && <p className="mt-0.5 break-words text-sm text-base-content">{event.detail}</p>}
        ...
        {shipment.pod_available && (
          <div className="flex flex-wrap items-start gap-3 rounded-xl border border-slate-200 bg-base-100 px-4 py-4 shadow-sm sm:px-5">
            <span className="flex h-9 w-9 flex-none items-center justify-center rounded-lg bg-slate-100">
              <Lock className="h-4 w-4 text-base-content" />
            </span>
            <div className="min-w-0">
              <p className="text-sm font-bold text-base-content">Proof of delivery is on file</p>
              <p className="mt-1 text-sm text-base-content">
