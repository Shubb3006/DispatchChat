...
const sizePx = (size) =>
  Number(String(size).match(/size-(\d+(?:\.\d+)?)/)?.[1] ?? 10) * 4;
...
const Avatar = ({
  ...
  /*
   * Reset image state whenever the avatar URL changes.
   * This is important when switching between conversations.
   */
  useEffect(() => {
  ...
  return (
    <div className={`relative shrink-0 ${className}`}>
      <div
        className={`${size} relative overflow-hidden rounded-full grid place-items-center
          select-none ring-1 ring-black/5 dark:ring-white/10
          ${ring ? "outline outline-2 outline-offset-2 outline-sky-500" : ""}`}
      >
