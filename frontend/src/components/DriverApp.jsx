...
import GPSMonitorWidget from "./driver/GPSMonitorWidget";
import ActiveLoadCard from "./driver/ActiveLoadCard";
import DocumentScannerWidget from "./driver/DocumentScannerWidget";
...
export default function DriverApp({
  ...
  const skidCanvasRef = useRef(null);
  ...
  const startSkidCamera = async () => {
    ...
    try {
      ...
      console.warn("Camera fallback to interactive simulation.", err);
      setCameraError(
        "Camera permission blocked or not supported inside sandbox frame. Activating Interactive Sandbox Camera Feed."
      );
    }
...
• Severity: HIGH (Distress Beacon Active)`,
  ...
  const handleSendDriverMessage = () => {
    ...
    if (!newMessage.trim() && !myShipment) return;
    console.log(newMessage);
    onSendMessage(
      newMessage,
      "DISP_OFFICE",
  ...
  const handleParseDocument = async () => {
    if (docType === "skid_picture" && skidPhotos.length === 0) {
      setOcrError("Please capture at least one skid condition photo.");
      return;
    }
    if (docType !== "skid_picture" && !fileText.trim() && !fileBase64) {
      setOcrError(
        "Please provide a document file or fallback text description to scan."
      );
      return;
    }
    ...
    const activeShipmentId =
      parserLoadId === "custom"
        ? "CUSTOM_LOAD"
        : parserLoadId || myShipment?.id || "SHP101";
    ...
    try {
      ...
      if (docType === "skid_picture") {
        resultData = {
          confidence: 98,
          palletCount: skidPhotos.length,
          condition: "Secure shrink wrap, intact corner guards, no tilt",
          items: `${skidPhotos.length} Pallet condition photos logged in sequential order`,
        };
      } else {
        resultData = {
          confidence: 96,
          bolNumber: "BOL-" + Math.floor(100000 + Math.random() * 900000),
          purchaseOrder: "PO-" + Math.floor(100000 + Math.random() * 900000),
          shipperName: myShipment?.shipperName || "AeroParts Hub",
          consigneeName: myShipment?.consigneeName || "Midwest Assembly",
          items: "Industrial Auto Component Castings",
          weightLbs: myShipment?.weightLbs || 8500,
          signatureFound: true,
        };
      }

      setExtractedResult(resultData);
      ...
      const newDoc = {
        ...
        fileName:
          fileName ||
          (docType === "skid_picture"
            ? `Skid_Photos_${skidPhotos.length}_Pallets.jpg`
            : "Scanned_Logistics_Receipt.pdf"),
        fileSize: "1.2 MB",
        uploadedBy: `${driverName} (Driver App)`,
        uploadDate: new Date().toISOString(),
        status: "pending_review",
        extractedData: resultData,
        skidPictures: docType === "skid_picture" ? [...skidPhotos] : undefined,
      };

      onAddDocument(newDoc);
    } catch (err) {
      console.error(err);
      setOcrError("Gemini OCR Scan failed. Using fallback parser.");
    } finally {
      setOcrLoading(false);
    }
  };
  ...
  const myDocuments = documents.filter(
    (d) =>
      ...
      d.uploadedBy?.includes(driverName) ||
      d.uploadedBy?.includes("Marcus")
  );

  const filteredDocLogs = myDocuments.filter((d) => {
    if (docLogFilter === "all") return true;
    if (docLogFilter === "approved")
      return d.status === "approved" || d.status === "matched_to_invoice";
    return d.status === docLogFilter;
  });

  const handleSelectAllFilteredDocs = (checked) => {
    if (checked) {
      setSelectedDocIds(filteredDocLogs.map((d) => d.id));
    } else {
      setSelectedDocIds([]);
  ...
  const handleBulkDownload = () => {
    const docsToDownload = myDocuments.filter((d) =>
      selectedDocIds.includes(d.id)
    );
    if (docsToDownload.length === 0) return;
    alert(
      `Downloading ${docsToDownload.length} selected logistics documents...`
    );
  };
  ...
  const handleDownloadDocument = (doc) => {
    ...
    a.href = getDocumentImage(doc);
    a.download = doc.fileName || "document.png";
  ...
  const maxWeightCapacity = 45000;
  const currentWeight = myShipment?.weightLbs || 8500;
  const scannedWeights = myDocuments
    .filter((d) => d.extractedData?.weightLbs)
    .map((d) => ({
      fileName: d.fileName,
      extractedWeight: Number(d.extractedData.weightLbs),
    }));
  const hasScannedWeight = scannedWeights.length > 0;
  const capacityUtilizedPercent = Math.min(
    Math.round((currentWeight / maxWeightCapacity) * 100),
  ...
  return (
    <div className="min-h-screen bg-slate-100/70 p-3 sm:p-6 space-y-5 text-slate-800">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {/* Duty Status Selector & Quick Controls */}
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-3xs font-bold font-mono text-slate-400 uppercase tracking-wider mr-1">
            Duty Status:
          </span>

          {[
            { key: "ON", label: "ON DUTY", color: "bg-emerald-600 text-white" },
            { key: "D", label: "DRIVING", color: "bg-indigo-600 text-white" },
            { key: "SB", label: "SLEEPER", color: "bg-amber-600 text-white" },
            { key: "OFF", label: "OFF DUTY", color: "bg-slate-700 text-white" },
          ].map((st) => {
            const isSelected = myHOSLog?.current_status === st.key;
            return (
              <button
                key={st.key}
                type="button"
      ...
      {activeTab === "load" && (
        <ActiveLoadCard
          ...
          hasScannedWeight={hasScannedWeight}
          currentWeight={currentWeight}
          maxWeightCapacity={maxWeightCapacity}
          capacityUtilizedPercent={capacityUtilizedPercent}
          scannedWeights={scannedWeights}
          isStopWithinRange={isStopWithinRange}
          getStopDistanceText={getStopDistanceText}
          handleCompleteStop={handleCompleteStop}
          setPendingPickupWaypointId={setPendingPickupWaypointId}
          setIsPickupModalOpen={setIsPickupModalOpen}
          setPendingDeliveryWaypointId={setPendingDeliveryWaypointId}
      ...
      {activeTab === "hos" && <HOSClockWidget myHOSLog={myHOSLog} />}
      ...
      <SOSBeaconModal
        ...
        sosSecondsLeft={sosCountdown}
        triggerSOS={startSOSCountdown}
        cancelSOS={cancelSOSAlert}
        confirmSOSNow={triggerSOSAlert}
      />

      <CameraBatchModal
        isSkidCameraOpen={isSkidCameraOpen}
        setIsSkidCameraOpen={setIsSkidCameraOpen}
        videoRef={videoRef}
        skidCanvasRef={skidCanvasRef}
        skidPhotos={skidPhotos}
        setSkidPhotos={setSkidPhotos}
        captureSkidPhoto={captureSkidPhoto}
        stopSkidCamera={stopSkidCamera}
        isBatchPreviewOpen={isBatchPreviewOpen}
        setIsBatchPreviewOpen={setIsBatchPreviewOpen}
        ...
        handleFinishSkidBatch={handleFinishSkidBatch}
        driverLocation={driverLocation}
        isMobileMode={isMobileMode}
      />

      <PickupDeliveryModals
        isPickupModalOpen={isPickupModalOpen}
        setIsPickupModalOpen={setIsPickupModalOpen}
        myShipment={myShipment}
        pickupError={pickupError}
        pickupLoadId={pickupLoadId}
        setPickupLoadId={setPickupLoadId}
        pickupCustomLoadNumber={pickupCustomLoadNumber}
        setPickupCustomLoadNumber={setPickupCustomLoadNumber}
        shipments={shipments}
        pickupBolFile={pickupBolFile}
        setPickupBolFile={setPickupBolFile}
        handleBolChange={handleBolChange}
        pickupSkidFile={pickupSkidFile}
        setPickupSkidFile={setPickupSkidFile}
        handleSkidPicChange={handleSkidPicChange}
        handleLoadDemoPickup={handleLoadDemoPickup}
        handleSubmitPickup={handleSubmitPickup}
        isDeliveryModalOpen={isDeliveryModalOpen}
        setIsDeliveryModalOpen={setIsDeliveryModalOpen}
        deliveryError={deliveryError}
        deliveryLoadId={deliveryLoadId}
        setDeliveryLoadId={setDeliveryLoadId}
        deliveryCustomLoadNumber={deliveryCustomLoadNumber}
        setDeliveryCustomLoadNumber={setDeliveryCustomLoadNumber}
        deliveryPodFile={deliveryPodFile}
        setDeliveryPodFile={setDeliveryPodFile}
        handlePodChange={handlePodChange}
        consigneeSignee={consigneeSignee}
        setConsigneeSignee={setConsigneeSignee}
        handleLoadDemoDelivery={handleLoadDemoDelivery}
      ...
      <DriverChatDrawer
        ...
        setNewMessageText={setNewMessage}
        handleSendMessage={handleSendDriverMessage}
