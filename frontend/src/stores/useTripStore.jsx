...
export const useTripStore = create((set, get) => ({
  ...
  fetchTrips: async () => {
    ...
    try {
      ...
      const parsedTrips = tripsData.map(item => typeof item.data === 'string' ? JSON.parse(item.data) : item.data || item);
      ...
      try {
        ...
        if (fallback && fallback.data) {
          const parsed = fallback.data.map(item => typeof item.data === 'string' ? JSON.parse(item.data) : item.data || item);
  ...
  addTrip: async (trip) => {
    set({ isLoading: true });
    try {
      const response = await axiosInstance.post('/trips', trip);
  ...
  updateTrip: async (trip) => {
    ...
    try {
      ...
      const updated = response.data || trip;
      set((state) => ({
