...
export default function SafetyPage() {
  ...
  // Fetch safety and HOS data on load of this specific page
  useEffect(() => {
        ...
        <div className="lg:col-span-8 space-y-6">
            ...
            <div className="px-5 py-4 border-b border-slate-150 bg-slate-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-base font-semibold text-slate-900">
                  Samsara Critical Safety Alerts & Incidents
                </h3>
              </div>
            ...
            <div className="divide-y divide-slate-100">
              {filteredIncidents.length === 0 ? (
                <div className="text-center text-slate-400 py-12 text-xs">
                  No active safety alerts triggered. Fleet is driving within
                ...
                filteredIncidents.map((inc) => (
                  <div
                    key={inc.id}
                    className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 hover:bg-slate-50/50 transition-colors"
                  >
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex items-center space-x-4 text-3xs font-mono text-slate-500 pt-1">
                          <span className="flex items-center">
                            <MapPin className="h-3 w-3 mr-1" /> {inc.location}
                          </span>
                          <span className="flex items-center">
                    ...
                    <div className="flex items-center space-x-2 shrink-0 self-end sm:self-start">
                      <span
                        className={`px-2.5 py-1 rounded text-2xs font-mono uppercase font-bold ${
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-4">
            <h3 className="text-sm font-semibold text-slate-900">
              Samsara Real-time ELD Duty Audit
            </h3>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-slate-100">
                  {hosLogs.map((log) => {
                    const isViolated =
                      log.drivingSecondsRemaining <= 0 ||
                      log.dutySecondsRemaining <= 0;
                    return (
                      <tr key={log.driverId} className="hover:bg-slate-50/50">
                        ...
                        <td className="px-4 py-3.5">
                          <span
                            className={`inline-flex items-center px-2 py-0.5 rounded text-2xs font-bold font-mono ${
                              log.currentStatus === "D"
                                ? "bg-indigo-100 text-indigo-800"
                                : log.currentStatus === "ON"
                                ? "bg-emerald-100 text-emerald-800"
                                : log.currentStatus === "SB"
                                ? "bg-amber-100 text-amber-800"
                        ...
                        <td className="px-4 py-3.5 font-mono">
                          {Math.floor(log.drivingSecondsRemaining / 3600)}h{" "}
                          {Math.floor(
                            (log.drivingSecondsRemaining % 3600) / 60
                          )}
                          m
                        </td>
                        <td className="px-4 py-3.5 font-mono">
                          {Math.floor(log.dutySecondsRemaining / 3600)}h{" "}
                          {Math.floor((log.dutySecondsRemaining % 3600) / 60)}m
                        </td>
                        <td className="px-4 py-3.5 font-mono">
                        ...
                        <td className="px-4 py-3.5 text-right">
                          {isViolated ? (
                            <span className="inline-flex items-center px-2 py-0.5 bg-rose-100 text-rose-800 text-3xs font-bold font-mono uppercase rounded">
                              Violation Active
                            </span>
                          ) : (
                            <span className="inline-flex items-center text-emerald-600 text-3xs font-bold font-mono">
                              Compliant
                            </span>
                          )}
                        </td>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 space-y-4">
            ...
            <div className="p-3 bg-indigo-50/50 border border-indigo-100 rounded-lg text-slate-600 text-3xs leading-relaxed">
              <span className="font-bold text-indigo-950 block mb-1 font-mono uppercase">
                Safety Policy Guidelines
              </span>
              All safety alerts trigger real-time notification warnings to
              driver ELD screens. Drivers holding safety scores below 80 are
              automatically prioritized for dispatcher route reviews and safety
              coaching.
            </div>
          </div>
        </div>
